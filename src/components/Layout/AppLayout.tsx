...
export default function AppLayout({ activeTab, onTabChange, children }: AppLayoutProps) {
    ...
    return (
        <div className="flex h-screen w-full bg-canvas text-primary font-sans overflow-hidden">
            ...
            <aside
                className={`
                    fixed inset-y-0 left-0 z-50 w-64 bg-sidebar border-r border-border dark:border-zinc-800 transform transition-transform duration-300 ease-in-out
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden bg-canvas relative">
                ...
                <div className="md:hidden h-16 flex items-center px-4 border-b border-border dark:border-zinc-800 bg-canvas/80 backdrop-blur-md sticky top-0 z-40">
                    <button
                        onClick={toggleSidebar}
