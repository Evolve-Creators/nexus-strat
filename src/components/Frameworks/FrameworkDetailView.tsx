...
export default function FrameworkDetailView({ framework, onBack }: FrameworkDetailViewProps) {
    ...
    return (
        <div className="h-full flex flex-col bg-zinc-950 text-zinc-100">
            {/* Header */}
            <div className="flex items-center gap-4 p-6 border-b border-zinc-800 bg-zinc-900/50 backdrop-blur-md sticky top-0 z-20">
                <button
                    onClick={onBack}
                ...
                <div className="ml-auto flex gap-2 items-center">
                    <span className="px-3 py-1 bg-zinc-900 border border-zinc-700 rounded-full text-xs text-zinc-400 capitalize mr-4">
            ...
            <div className="flex-1 overflow-hidden flex flex-row relative">
                ...
                <div className="flex-1 bg-zinc-950 relative overflow-auto h-full w-full">
                    {/* Centered Scrollable Container */}
                    <div className="min-h-full min-w-full flex items-start justify-center p-8">
                        <div className="w-full max-w-[1200px] bg-zinc-900/10 border border-zinc-800 rounded-2xl p-4 md:p-8 shadow-2xl relative">
                            {/* Diagram Component */}
                            <FrameworkDiagram
                                framework={framework}
                                content={{}}
                ...
                <div
                    className={`
                        border-l border-zinc-800 bg-zinc-900/30 overflow-y-auto 
                        ...
                        {framework.sections.find(s => s.id === 'preliminary') && (
                            <section>
                                ...
                                <div className="bg-purple-900/10 border border-purple-900/30 rounded-lg p-4">
                                    <ul className="space-y-2 text-sm text-purple-200/80 list-disc list-inside">
                            ...
                            <section>
                                ...
                                <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-2">
                                    {framework.equations.map((eq, i) => (
                                        <div key={i} className="font-mono text-xs text-zinc-400 border-b border-zinc-800/50 last:border-0 pb-2 last:pb-0">
                                            {eq}
                                        </div>
                                    ))}
                                </div>
                        ...
                        <section>
                            ...
                            <ul className="space-y-2">
                                {framework.sections.map(section => (
                                    <li key={section.id} className="text-xs text-zinc-400 flex gap-2">
                                        <span className="text-zinc-600">•</span>
                                        <span>
                                            <strong className="text-zinc-300">{section.name}</strong>
                                            {section.description && <span className="text-zinc-600"> - {section.description}</span>}
                                        </span>
                                    </li>
                                ))}
                            </ul>
